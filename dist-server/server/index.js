...
function computeStats(orders, periodLabel) {
    ...
    const fruitStats = Array.from(fruitMap.entries())
        .map(([name, data]) => ({
        fruitName: name,
        emoji: orders_1.FRUIT_EMOJIS[name] || "🍈",
...
app.get("/api/dates", (req, res) => {
    const dates = (0, orders_1.getAvailableDates)();
    res.json({ dates });
});
app.get("/api/orders/day", (req, res) => {
    ...
    const orders = (0, orders_1.getDayOrders)(date);
...
app.get("/api/orders/week", (req, res) => {
    ...
    const orders = (0, orders_1.getWeekOrders)(startDate);
